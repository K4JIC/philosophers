...
int	philo_eat(t_philo_thread_info *philo_info)
{
	...
	pthread_mutex_lock(philo_info->last_eat_clock_us->mu);
	if (get_time_us(philo_info->last_eat_clock_us->val) == GET_TIME_ERROR)
		return (GET_TIME_ERROR);
	pthread_mutex_unlock(philo_info->last_eat_clock_us->mu);
	if (philo_write(philo_info, "has taken a fork") == GET_TIME_ERROR)
		return (GET_TIME_ERROR);
